...
/**
 * This implements the GraphHopper Routing Engine using Flexible Routing to incorporate Turn-Restrictions
 */
public class GraphHopperRouter implements RoutingInterface {
    ...
    // Singleton
	private static GraphHopperRouter INSTANCE = new GraphHopperRouter();


	/**
	 * Private constructor for GraphHopperRouter.
	 * Call GraphHopperRouter.getInstance() to obtain a instance of this class.
	 */
	private GraphHopperRouter() {
        ...
        hopper.setGraphHopperLocation(Config.GRAPHHOPPER_FOLDER_GRAPH);
        hopper.setMinNetworkSize(200, 200);

		hopper.setCHEnabled(false);
		hopper.setEncodingManager(EncodingManager.create("car|turn_costs=true,foot"));
	 ...
	 */
	public static GraphHopperRouter getInstance() {
		return INSTANCE;
	}
	...
	private PathWrapper calculatePath(Position from, Position to, SimObjectRoutable.Type vehicleType) throws RoutingException {
		GHRequest request;
		if (from.getHeading() != null) {
			request = new GHRequest().addPoint(new GHPoint(from.getY(), from.getX()), from.getHeading()).addPoint(new GHPoint(to.getY(), to.getX()));
		} else {
	...
	public RouteStepEnroute.EnrouteTrack calculateRoute(Position from, Position to, SimObjectRoutable.Type vehicleType, long startTime) throws RoutingException {
		if (Config.ENABLE_TURN_RESTRICTIONS_AND_HEADING) {
			try {
				// Calculate Route
				...
				for (TimePosition entry : pathTime) {
					trackMap.put(
				...
				// Fallback to GraphHopper with Contraction Hierachies in case of errors
				Logger.trace("Routing with CityGrid failed, fallback to GraphHopperCH");
				return GraphHopperRouterCH.getInstance().calculateRoute(from, to, vehicleType, startTime);
			}
		} else {
			return GraphHopperRouterCH.getInstance().calculateRoute(from, to, vehicleType, startTime);
		}
	 ...
	 */
	private List<TimePosition> interpolateGPXTimes(List<GPXEntry> gpxPath) {
		Long stepBegin = 0L;

		List<TimePosition> finalList = new ArrayList<>();
		List<TimePosition> tempList = new ArrayList<>();

		for (GPXEntry entry : gpxPath) {
			if (entry.getTime() != null && tempList.size() == 0) {
				stepBegin = entry.getTime();
				tempList.add(new TimePosition(entry.getTime(), new Position(entry.getPoint().getLon(), entry.getPoint().getLat())));
			} else if (entry.getTime() == null) {
				tempList.add(new TimePosition(null, new Position(entry.getPoint().getLon(), entry.getPoint().getLat())));
			} else {
				...
				tempList.clear();
				tempList.add(new TimePosition(stepEnd, new Position(entry.getPoint().getLon(), entry.getPoint().getLat())));
				stepBegin = stepEnd;
			}
		}
		finalList.addAll(tempList);
	 ...
	 */
	private void updateHeadingInfo(List<TimePosition> timePositionList) {
